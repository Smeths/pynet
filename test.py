...
wih_data_file = open("data/weights/wih.csv",'r')
wih_data = wih_data_file.read().split()
wih_data = [float(i) for i in wih_data]
n.wih = numpy.array(wih_data).reshape(hidden_nodes, input_nodes)
wih_data_file.close()

# overwriting the weights used in the network
who_data_file = open("data/weights/who.csv",'r')
who_data = who_data_file.read().split()
who_data = [float(i) for i in who_data]
n.who = numpy.array(who_data).reshape(output_nodes, hidden_nodes)
who_data_file.close()
